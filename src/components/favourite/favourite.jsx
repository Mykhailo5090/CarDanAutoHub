...
const FavoritesPage = () => {
  ...
  const handleDeleteSelected = async () => {
    if (window.confirm(`Видалити вибрані авто (${selectedIds.length}) з обраного?`)) {
      await Promise.all(
  ...
  const handleCompareSelected = () => {
    const carsToCompare = favorites.filter((car) => selectedIds.includes(car.id));
    ...
    return (
      <div className="__wrapper_buypage comparison-page-view">
        <div className="comparison-page-header">
          <button className="bulk-btn btn-back" onClick={() => setIsComparing(false)}>
            ← Back to Favourite's
          </button>
        ...
        <div className="comparison-board-wrapper __shadows">
          <table className="comparison-board-table">
            <thead>
              <tr>
                <th className="sticky-spec-column"> <strong>Car:</strong> </th>
                {compareList.map((car) => {
                  const images = JSON.parse(car.images);
                  return (
                    <th key={car.id} className="car-header-cell">
                      <button className="remove-from-comp-btn" onClick={() => handleRemoveFromCompare(car.id)}>
                        ✕ Delete
                      </button>
                      <div className="comp-card-preview">
                        <img src={`http://localhost:5001${images[0]}`} alt="car" className="comp-card-img" />
            ...
            <tbody>
              <tr>
                <td className="sticky-spec-column"><strong>Year:</strong></td>
                {compareList.map(car => <td key={car.id} className="comp-data-cell">{car.year}</td>)}
              </tr>
              <tr>
                <td className="sticky-spec-column"><strong>Mileage</strong></td>
                {compareList.map(car => <td key={car.id} className="comp-data-cell">{car.mileage} km</td>)}
              </tr>
              <tr>
                <td className="sticky-spec-column"><strong>Engine / Fuel</strong></td>
                {compareList.map(car => <td key={car.id} className="comp-data-cell">{car.engine_volume}L ({car.fuel_type})</td>)}
              </tr>
              <tr>
                <td className="sticky-spec-column"><strong>Gear-Box</strong></td>
                {compareList.map(car => <td key={car.id} className="comp-data-cell">{car.transmission}</td>)}
              </tr>
              <tr>
                <td className="sticky-spec-column"><strong>Location</strong></td>
                {compareList.map(car => <td key={car.id} className="comp-data-cell">{car.region}</td>)}
              </tr>
              <tr>
                <td className="sticky-spec-column"><strong>Phone Number</strong></td>
                {compareList.map(car => <td key={car.id} className="comp-data-cell">{car.owner_phone || "Не вказано"}</td>)}
              </tr>
              <tr>
                <td className="sticky-spec-column"><strong>Seller</strong></td>
                {compareList.map(car => (
                  <td key={car.id} className="comp-data-cell">
                    <div className="comp-seller-info">
                      {car.owner_avatar ? (
                        <img src={`http://localhost:5001${car.owner_avatar}`} alt="avatar" className="comp-seller-avatar" />
                      ) : "👤 "}
              ...
              <tr>
                <td className="sticky-spec-column"><strong>Description from the owner:</strong></td>
                {compareList.map(car => (
        ...
        <div className="sub-body__2">
          <div className="__wrapper_buypage">
            ...
            <CarFilters 
              ...
              isAllSelected={selectedIds.length === filteredFavorites.length && filteredFavorites.length > 0}
            />

            {/* Грід з картками */}
            <div className="cars-grid">
              {filteredFavorites.length === 0 ? (
                <p className="p_alert_favourites" style={{ gridColumn: '1/-1', textAlign: 'center' }}>
